...
public abstract class PromptAiEngine implements AiEngine {
    ...
    public String planNextRequest(String goal, String observation, String feedback) {
        String system =
                ...
                + "  returned-content SSRF: a request field holds a URL/image/link/file (name or value looks like a URL\n"
                + "    or path); set that field to an internal URL on the SAME host (e.g. http://<host>/<internal-path>)\n"
                + "    so the server fetches it and may EMBED the fetched content in its response (sometimes base64).\n"
                + "  credential chaining: if a response — or an already-decoded base64 field shown in the observation —\n"
                + "    reveals a password/secret/credential, your NEXT step is to AUTHENTICATE at the login/token\n"
                + "    endpoint with it, then reuse the returned token as Authorization: Bearer to retry anything that\n"
                + "    previously returned 401/403 (privilege escalation via leaked credential).\n"
                + "  reach-403: if an endpoint returned 401/403, try it again after any step that yields a fresh token.\n"
                + "You do NOT decide whether anything is vulnerable — a deterministic oracle does; your job is targeting.\n"
                + "RULES: build requests from paths, params, ids and tokens that appear in the observation OR the goal's\n"
                + "source-code leads — do not invent a brand-new endpoint to REQUEST directly (an invented target is\n"
                + "live-probed and discarded). EXCEPTION: as a VALUE for a url/image/link/redirect field you MAY use an\n"
                + "internal URL on the same host even if that internal path is not in the observation — that is the SSRF\n"
                + "probe itself, not an invented request target. Keep the SAME host. When probing SQLi/XSS/SSTI/traversal,\n"
                + "embed the exact canary tokens named in the goal so the oracle can confirm.\n"
